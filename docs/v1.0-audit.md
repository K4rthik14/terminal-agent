...
- **Terminal-Bench results**: the repository's `jobs/` directory contains a Terminal-Bench-2 *oracle-agent* harness-validation run (gold patches), not a Reflex run. **No Terminal-Bench score for Reflex exists in the repository.**
- Persistent memory (`memory/` is a stub), self-improvement loops, semantic retrieval.
...
- Limitations: tiny suite, exit-code-only judging, no parallelism, no regression tracking, no statistical treatment. **No external benchmark results are claimed.** The `jobs/` Terminal-Bench artifacts are oracle-agent (gold-patch) validation runs of the harness itself.
...
3. `MultiAgentCoordinator` unwired; planner placeholder; memory stub.
...
2. **Memory/state:** persistent cross-session project memory replacing the stub.
